# StackHawk SME Training: HawkScan Deep Dive (Windows Guide)
...
> [!NOTE]
> All commands in this guide are written for **PowerShell**. If you have not yet completed the pre-session prerequisites, return to the [pre-session setup page](README.md) before continuing.
...
Time to get our target application running. Make sure you are in your `stackhawk-training` directory before running this command:

```powershell
Start-Process java -ArgumentList "-jar","java-spring-vuly-0.2.0.jar","--spring.profiles.active=windows"
```

This starts JavaSpringVulny in a new background process. Once started, the application is accessible at [https://localhost:9000](https://localhost:9000).
...
Spend a moment exploring the app. Look at the login form, the different pages, and the API endpoints. This is what we will be scanning.

---

## Step 4: _YAML Deep Dive, API Spec + Authentication_

This is the heart of today's session. We are going to walk through `stackhawk-training.yml` in detail. This single file handles authentication and API spec-driven scanning together.
...
```powershell
$env:APP_ID = "XXXXX"; hawk scan stackhawk-training.yml
```
...
```
%USERPROFILE%\.hawk\sessions\<scan-id>\hawkscan.log
```
...
```powershell
$env:USE_NATIVE_TRUSTSTORE = "true"
